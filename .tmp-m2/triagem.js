...
const div = (cnae) => (cnae || "").replace(/\D/g, "").slice(0, 2);
/**
 ...
 */
export function anexoPorCnae(cnae) {
    const d = div(cnae);
    if (!d)
        return undefined;
    const n = Number(d);
    if (["45", "46", "47"].includes(d))
        return 1; // comércio
    if (n >= 5 && n <= 33)
        return 2; // indústria e extrativa
    if (["41", "42", "43"].includes(d))
        return 4; // construção civil
    if (n >= 49 && n <= 99)
        return 3; // serviços em geral
    return 3;
}
export function triar(e) {
...
export function resumir(triagens) {
    const base = { MEI: 0, FORA: 0, A: 0, B: 0, C: 0, D: 0 };
