 ...
 */
export function situacaoPlano(assinatura, laudosEmitidos) {
...
export function mensagemBloqueio(s) {
    return `Você já emitiu ${s.usados} de ${s.limite} laudos do plano gratuito. Assine o PRO para emitir laudos e termos ilimitados.`;
}
const moedaBR = (v) => `R$ ${v.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
export function montarMuro(s, honorario, precoAnualCentavos) {
    const anual = precoAnualCentavos ? precoAnualCentavos / 100 : null;
    const conta = anual && anual > 0 && honorario > 0
    ...
    return {
        ...
        linhas: conta
            ...
            : ["Uma única análise cobrada do seu cliente paga o Enquadria por um ano inteiro."],
        conta,
        nota_anual: "A opção é semestral: esta janela fecha em 30/09 e a mesma carteira volta à mesa em março de 2027. O anual cobre as duas.",
...
export function avisoLimite(s) {
    if (s.ilimitado || s.bloqueado)
        return null;
    if (s.restantes === 1)
        return "Resta 1 laudo do plano gratuito.";
    if (s.restantes <= 2)
        return `Restam ${s.restantes} laudos do plano gratuito.`;
    return null;
}
