 ...
 */
import { pct, moeda, SAIDAS, ANEXOS_SIMPLES, } from "./motor";
/**
 ...
 * que `re`, porque `rq < 1`.
 */
export function reLiquidoDe(a) {
    const aliq = a.parametros?.aliquota;
    if (a.re == null || aliq == null)
        return null;
    return Number(a.re) * (1 - Number(aliq));
}
export function reUnicoDe(a) {
    return a.cl == null ? null : Number(a.cl);
}
export function premissasEstimadas(a) {
...
export function premissasEmTexto(r) {
    if (!r)
        return [];
    const linhas = [];
    if (r.b2b != null)
        linhas.push(`Vendas para pessoa jurídica: ${FAIXA_LABEL[String(r.b2b)] ?? pct(r.b2b)} da receita`);
    if (r.qual != null)
        linhas.push(`Clientes PJ que aproveitam crédito: ${FAIXA_LABEL[String(r.qual)] ?? pct(r.qual)}`);
    if (r.cred != null)
        linhas.push(`Compras que geram crédito: ${FAIXA_LABEL[String(r.cred)] ?? pct(r.cred)} da receita`);
    if (r.preco != null) {
    ...
    if (r.exig === 1)
        linhas.push("Cliente PJ já sinalizou que exigirá crédito integral em 2027");
    return linhas;
}
export function resultadoEmTexto(a) {
    const linhas = [];
    if (a.fc != null)
        linhas.push(`Crédito transferido ao comprador: ${pct(Number(a.fc))} da operação`);
    if (a.re != null)
        linhas.push(`Repasse de preço necessário: ${pct(Number(a.re))}`);
    /**
    ...
    if (unico != null && unico > 0) {
        linhas.push(`Com uma tabela de preço só: ${pct(unico)} em todos os preços — menor, ` +
            "porém cobrado também de quem não aproveita crédito");
    }
    return linhas;
}
export function pressaoDoLaudo(a) {
    const p = a.parametros ?? {};
    const aliq = p.aliquota;
    if (a.re == null || a.fc == null || a.cl == null || a.rq == null || aliq == null)
        return null;
    const re = Number(a.re), fc = Number(a.fc), cl = Number(a.cl), rq = Number(a.rq);
    if (!isFinite(re) || cl <= 0 || fc <= 0)
        return null;
    ...
    if (rq < (typeof p.rqMin === "number" ? p.rqMin : 0.3))
        return null;
    ...
    const nivel = parte >= 0.75 ? "apertada" : parte >= 0.5 ? "media" : "folgada";
    const leitura = nivel === "apertada"
    ...
    if (a.respostas?.conc === 1) {
        avisos.push("Os concorrentes desta empresa estão majoritariamente fora do Simples, e por isso já entregam " +
            "crédito integral. Aqui a opção não cria vantagem — reduz uma desvantagem, e o argumento de " +
            "preço perde força mais rápido.");
    }
    else {
        avisos.push("Gerar crédito integral é vantagem enquanto os concorrentes não geram. Quando eles optarem, a " +
    ...
    if (a.respostas?.preco != null && Number(a.respostas.preco) <= 1) {
        avisos.push("A empresa declarou não ter poder de renegociar preço. Nesse caso o cenário realista não é o " +
...
export function absorcaoDoLaudo(a) {
    const p = a.parametros ?? {};
    const preco = a.respostas?.preco;
    if (preco == null || Number(preco) > 1)
        return null;
    if (a.cl == null || a.fc == null || a.rq == null)
        return null;
    ...
    return {
        ...
        entrega: pct(fc),
        pergunta: `${pct(cl, 2)} da receita cabe na margem desta empresa? Este laudo conhece a receita e não ` +
...
export const FRONTEIRA_CONTA_NEGOCIACAO = "Até aqui é conta: os números acima decorrem das premissas informadas e da legislação citada, e " +
...
export function recomendacao(a) {
    const s = (a.saida ?? "S1");
    return SAIDAS[s];
}
/**
 ...
 */
export function baseDeCalculo(a) {
    const d = a.parametros?.ddas;
    if (!d)
        return [];
    const linhas = [];
    if (d.fonte === "efetiva") {
        linhas.push(`Alíquota efetiva do Simples: ${pct(d.aliquota)} — apurada sobre a RBT12 de ${moeda(d.rbt12)} (Anexo ${d.anexo}, faixa ${d.faixa}).`);
    }
    else {
        linhas.push(`Alíquota do Simples: ${pct(d.aliquota)} — topo da faixa ${d.faixa} do Anexo ${d.anexo}, ` +
     ...
     */
    linhas.push(`Parcela que sai do DAS ao optar: ${pct(d.sharePC)} da carga do Simples = ${pct(d.das)} da receita.`);
    /**
     ...
     */
    if (d.teto_iss) {
        linhas.push(`Teto de 5% do ISS aplicado: pela tabela do anexo o ISS efetivo seria ${pct(d.teto_iss.iss_sem_teto, 2)}, ` +
        ...
        linhas.push("Base: nota de rodapé dos Anexos XX e XXI da Lei Complementar nº 214/2025 — o ISS fica fixo em 5% e " +
            "os demais tributos recebem (alíquota efetiva − 5%) × percentual de redistribuição da faixa.");
    }
    linhas.push("Essa parcela corresponde às colunas de CBS e IBS da partilha do anexo (Anexos XVIII a XXII da " +
 ...
 */
export const NOTA_PARAMETROS = "Os cortes usados na recomendação — receita qualificada mínima de 30%, banda de fronteira de 0,8 a " +
...
export function dDASestimado(a) {
    return a.parametros?.ddas?.fonte === "conservador";
}
export function decisaoSugerida(a) {
...
export function ehLaudoCurto(faixa) {
...
export function premissasComOrigem(a) {
    const r = a.respostas ?? {};
    const p = a.parametros ?? {};
    const o = (k) => {
        const v = p.origens?.[k];
        if (v === "coleta" || v === "informada" || v === "estimada" || v === "padrao")
            return v;
 ...
 */
export function memoriaDeCalculo(a) {
    const p = a.parametros ?? {};
    const d = p.ddas;
    const r = a.respostas ?? {};
    const passos = [];
    const n = (x, casas = 4) => x == null || !isFinite(x) ? "—" : x.toFixed(casas).replace(".", ",");
    /**
    ...
    if (d && segs && segs.length > 1) {
        segs.forEach((s, i) => {
            ...
            passos.push({
                ...
                substituicao: s.fonte === "efetiva" && s.rbt12 && t
...
export function condicoesDeValidade(a) {
    const r = a.respostas ?? {};
    const p = a.parametros ?? {};
    const cond = [];
    if (a.rq != null) {
        cond.push(`A receita vendida a quem aproveita crédito permanecer em torno de ${pct(Number(a.rq))} do faturamento.`);
    }
    if (r.cred != null) {
    ...
    if (a.re != null && isFinite(Number(a.re)) && ehOptarSaida(a.saida)) {
        cond.push(`O reajuste de preço de ${pct(Number(a.re))} ser efetivamente aceito pelos clientes empresa antes do fim da janela.`);
    }
    if (p.carimbo) {
        cond.push(`A alíquota de referência de IBS/CBS ser fixada em patamar próximo de ${pct(p.carimbo.aliquota)} — o cenário alternativo de ${pct(p.carimbo.alternativa)} está na seção de sensibilidade.`);
    }
    if (p.rbt12 != null && p.sublimite) {
        cond.push(`A receita do ano permanecer do mesmo lado do sublimite de ${moeda(p.sublimite)}, que altera o que já sai do DAS.`);
    }
    /**
    ...
    if (segs && segs.length > 1) {
        cond.push("A composição da receita permanecer próxima da declarada — " +
        ...
        if (segs.some((s) => s.anexo === 3 || s.anexo === 5)) {
            cond.push("O fator R do período manter a receita de serviço no anexo declarado — a folha em relação " +
...
export function riscosELimites(a) {
    ...
    if (p.ddas?.fonte === "conservador") {
        riscos.push("A RBT12 não foi informada: a parcela que sai do DAS foi estimada pelo topo da faixa, o que tende a superestimar o custo do regime regular. Informar a receita dos últimos 12 meses torna o número exato.");
    }
    if (p.origem_premissas === "lote_cnae") {
        riscos.push("As premissas deste laudo foram estimadas a partir do CNAE na análise em lote e não foram confirmadas caso a caso.");
    }
    if (p.fator_r) {
        riscos.push(`Fator R e anexo declarado divergem: ${p.fator_r.texto} ${p.anexo_confirmado ? "O anexo foi confirmado pelo contador responsável." : "O anexo ainda não foi confirmado."}`);
    }
...
export function tabelaDoAnexo(a) {
    const d = a.parametros?.ddas;
    if (!d)
        return null;
...
export const BASE_LEGAL = [
    ...
    },
    {
        ...
        papel: "regulamentou o IBS e a CBS e disciplinou a apuração pelo optante do Simples, dentro ou fora do documento único de arrecadação. O art. 41, § 3º cria a faculdade de apurar pelo regime regular; o § 5º veda a saída a quem recebeu ressarcimento de créditos.",
    },
    {
        norma: "Lei Complementar nº 214/2025, art. 519",
        papel: "substituiu os Anexos I a V da Lei Complementar nº 123/2006 pelos Anexos XVIII a XXII, com efeitos a partir de 1º/01/2027. É deles que sai a parcela usada neste laudo: as colunas de Cofins e PIS/Pasep dão lugar às de CBS e IBS, de soma idêntica nas cinco primeiras faixas.",
    },
    {
        norma: "Lei Complementar nº 214/2025, arts. 344 e 347",
        papel: "fixaram, para 2027 e 2028, o IBS em 0,05% estadual mais 0,05% municipal e a CBS na alíquota de referência reduzida em 0,1 ponto percentual. São a origem da alíquota usada no cenário principal.",
    },
    {
        norma: "Lei Complementar nº 214/2025, art. 47, § 9º, II",
        papel: "é o que sustenta a conta do outro lado do balcão: o adquirente sujeito ao regime regular credita, na compra de optante do Simples, montante equivalente ao devido por meio desse regime. Quando o fornecedor opta, o crédito passa a ser o do regime regular — e a diferença entre os dois é o ganho do comprador calculado aqui.",
    },
    {
        norma: "Lei Complementar nº 123/2006, art. 13, §§ 9º e 10",
        papel: "é o dispositivo operativo: faculta ao optante apurar IBS e CBS pelo regime regular, hipótese em que as parcelas relativas a eles não são cobradas pelo regime único, e fixa a opção como semestral e irretratável, exercida em setembro e em março. Redação dada pela Lei Complementar nº 227/2026.",
    },
    {
        norma: "Lei Complementar nº 227/2026",
        papel: "revogou o art. 87-B e postergou o art. 517 da LC 214/2025, deslocando o fundamento da regulamentação da opção.",
    },
    {
        norma: "Resolução CGSN nº 186/2026",
        papel: "abriu a janela de 1º a 30 de setembro de 2026 para a opção por apurar IBS e CBS fora do DAS, com efeito de janeiro a junho de 2027 e cancelamento até o último dia de novembro de 2026. Com a revogação do art. 87-B, a Resolução apoia-se no art. 41, §§ 3º e 4º, da Lei Complementar nº 123/2006.",
    },
];
