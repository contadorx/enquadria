 ...
 */
import { decidir, dDASefetivo, faixaDe, moeda, pct, } from "./motor";
/** sublimite de ICMS/ISS do Simples Nacional */
export const SUBLIMITE = 3600000;
 ...
 */
export function projetarRBT12(e) {
    const meses = e.meses ?? MESES_ATE_FIM_DO_EFEITO;
    if (!(e.rbt12 > 0) || !(meses > 0))
        return null;
    let crescimento;
    let origem;
 ...
 */
export function decidirComProjecao(r, p, proj) {
    ...
    linhas.push(proj.origem === "medido"
    ...
    if (proj.muda_faixa) {
        linhas.push(`A empresa muda da faixa ${proj.faixa} para a faixa ${proj.faixa_projetada} dentro do ` +
        ...
        linhas.push(`A faixa não muda dentro do período de efeito: a parcela que sai do DAS permanece em ` +
            `${pct(proj.das, 2)} da receita.`);
    }
    /* os dois cruzamentos, em ordem de gravidade */
    if (proj.acima_do_teto_hoje) {
        linhas.push(`A RBT12 de hoje (${moeda(proj.rbt12)}) já supera o teto do Simples Nacional de ` +
            `${moeda(TETO_SIMPLES)}. Não há decisão de setembro a tomar: a empresa não é optante.`);
    }
    else if (proj.cruza_teto) {
        linhas.push(`ALERTA — a projeção ultrapassa o teto do Simples Nacional (${moeda(TETO_SIMPLES)}) dentro ` +
    ...
    if (proj.cruza_sublimite) {
        linhas.push(`ALERTA — a projeção ultrapassa o sublimite de ${moeda(SUBLIMITE)} dentro do período de ` +
    ...
    if (divergem) {
        ...
        saida = hoje.saida;
        motivo = hoje.motivo;
        linhas.push("As duas contas — RBT12 de hoje e RBT12 projetada — levam à mesma saída. Como a RBT12 " +
