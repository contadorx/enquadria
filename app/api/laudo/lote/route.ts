...
export async function POST(req: Request) {
  ...
  let q = supabase.from("analises").select("id, empresa_id");
  ...
  if (!analises?.length) {
    return NextResponse.json({ ok: true, emitidos: 0, ja_tinham: 0, bloqueados: 0 });
  }
  ...
  const pendentes = analises.filter((a) => !jaTem.has(a.id));
