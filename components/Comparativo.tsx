...
/**
 * COMPARATIVO DE REGIMES — agora uma aba da gaveta, não uma tela do menu.
 *
 * Depois de setembro a pergunta deixa de ser "opto pelo híbrido?" e vira "em
 * que regime este cliente deveria estar?". A calculadora é a mesma; o que mudou
 * é que ela abre em cima da fila, com a empresa já carregada, e o documento
 * emitido volta para o dossiê ao lado.
 */
...
export function Comparativo({
  ...
  nome: string;
  anexoInicial: number | null;
  receitaInicial: number | null;
}) {
  const [setor, setSetor] = useState<Setor>("comercio");
  const [anexo, setAnexo] = useState(anexoInicial ?? 1);
  const [receita, setReceita] = useState(receitaInicial != null ? String(receitaInicial) : "1200000");
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-[12.5px] text-muted">
          Simples × Presumido × Real no mundo IBS/CBS — estimativa de cenário para {nome || "esta empresa"}.
        </p>
        <button
          onClick={emitir}
