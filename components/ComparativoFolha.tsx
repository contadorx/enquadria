...
/**
 * A FOLHA DO COMPARATIVO — só apresentação.
 *
 * Extraída da rota pelo mesmo motivo do LaudoFolha: o documento agora tem DOIS
 * endereços. `/doc/comparativo/[id]` lê com a sessão do contador;
 * `/comparativo/[token]` é o link que vai ao cliente e lê pelo token. Se o
 * desenho continuasse dentro da rota, existiriam duas versões do mesmo
 * documento e elas divergiriam na primeira alteração — o cliente receberia um
 * comparativo diferente do que o contador revisou.
 *
 * `publico` só governa a navegação: quem chega pelo link não tem painel para
 * onde voltar. O CONTEÚDO é idêntico nos dois — entregar menos ao cliente
 * esvaziaria o documento que justifica o honorário.
 */

export interface DadosComparativo {
  numero: number;
  emitido_em: string;
  entrada: ResultadoComparativo["entrada"];
...
export function ComparativoFolha({
  ...
  const { empresa, escritorio: t } = dados;
  ...
  return (
    <div className="doc">
      <div className="no-print mb-4 flex items-center justify-between">
        {publico ? <span /> : <Link href="/painel" className="text-sm text-accentdeep">← voltar</Link>}
        <BotaoImprimir />
      </div>
